...
def canonicalize_rev_value(v: str) -> str:
    ...
    if s in {"", "NO_REV", "NONE", "N/A", "OF", "O/F"}:
        return "NO_REV"
...
def is_empty_rev_value(v: str) -> bool:
    """
    True if v represents an explicitly blank/empty revision.
    This includes NO_REV, OF (canonicalized), and dash/underscore placeholders.
    """
    s = canonicalize_rev_value(v)
    return s in {"NO_REV", "-", "_", ".-", "._"}
...
def is_suspicious_rev_value(v: str) -> bool:
    """
    Returns True for values that should trigger a second opinion (e.g. GPT) or review.

    Suspicious includes:
      - pure numeric values (e.g. "202")
      - hyphenated numeric values where the suffix is non-zero (e.g. "5-40")
      - double letters where first letter >= D (e.g. "DP")
      - alphabetic strings length>=3 (e.g. "LTR") which are unlikely to be REV
      - anything that looks like a REV token but is out-of-scope
    """
    s = canonicalize_rev_value(v)

    # Empty/blank markers are valid and should not be treated as suspicious by default
    if is_empty_rev_value(s):
        return False

    # Catch unlikely alphabetic outputs like "LTR"
    if s.isalpha() and len(s) >= 3:
        return True

    if not REV_VALUE_RE.fullmatch(s):
        # If it doesn't even look like a REV token, it's not actionable here.
        return False

    if NUMERIC_ONLY_RE.fullmatch(s):
        return True

    # Anything that fails plausibility checks should be treated as suspicious
    if not is_plausible_rev_value(s):
        return True
