...
def _pick_latest_in_title_block(
    ...
    Rule established for this site:
      ...
      - Some drawings legitimately have NO revision yet, indicated by ``-``,
        ``_`` or similar dash characters in the rev cell. These are accepted
        as the value when no VALUE_RE candidate sits directly below the
        marker.

    Algorithm:
      1. Cluster VALUE_RE candidates by row below the marker. Take the TOP-most
         cluster (the cell directly under the marker). Stacking detection
         requires BOTH tight y-spacing AND tight x-alignment, so the
         ``Sheet N of M`` row in the next cell down (different x positions,
         one full cell-height below) is correctly excluded.
      2. If no VALUE_RE candidate sits in the primary cell (i.e. the closest
         candidate is more than ~1.8 row-heights below), look for a dash
         token (``-``, ``_``, ``−``, ``—``, ``–``) directly below the marker.
         This handles new drawings where the rev cell shows ``-`` instead
         of a numbered revision.

    The ``tokens`` parameter is needed only for the dash-fallback path
    (which inspects all tokens, not just VALUE_RE candidates). It is
    optional for backwards compatibility — callers that don't pass it
    simply lose the dash-fallback capability.
    """
    # Title-block column width tolerance.
    col_tol = max(marker.w * 1.2, marker.h * 3.0, 18.0)
    # The "primary cell" sits within ~1.8 row-heights of the marker. Anything
    # further down is the NEXT cell (typically "Sheet N of M" or footer text).
    # We tighten this from the prior 8.0 multiplier so that values >12 px below
    # the marker — which on this site's templates are reliably "next cell"
    # entries — are correctly excluded.
    # Rationale for 1.8x: typical cell heights on Rotork modern templates are
    # 12-16 px. The value sits at ~9-11 px below the marker. A 1.8x multiplier
    # of max(marker.h, 10) gives ~18 px, which captures the value but excludes
    # the next-row content at ~12-16 px below the value (i.e. ~21-27 px below
    # the marker).
    primary_cell_max_dy = max(marker.h, 10.0) * 1.8
    ...
    if in_column:
        ...
        primary = in_column[0]
        # Only treat the closest candidate as a "primary cell" hit if it's
        # within the primary-cell window. Otherwise the closest VALUE_RE
        # candidate is something like Sheet/footer text in a lower cell —
        # do not return it.
        if primary.y - marker.y <= primary_cell_max_dy:
            # In-cell stacking detection now requires BOTH:
            #   (a) tight y-spacing relative to the primary token's height
            #       — same cell means values share the cell's vertical space,
            #   (b) tight x-alignment to the primary token
            #       — same cell means values are written in the same column.
            # The previous logic used only condition (a) with a fixed 12 px
            # threshold, which wrongly grouped the "Sheet 1 of 1" row
            # (sitting at exactly 12 px below the primary on Losser-template
            # drawings) into the same "cell" as the primary.
            stack_y_threshold = max(marker.h, primary.h) * 1.3
            stack_x_threshold = max(min(primary.w, marker.w) * 0.6, 4.0)
            stacked = [
                c for c in in_column
                if c.y - primary.y <= stack_y_threshold
                and abs(c.x - primary.x) <= stack_x_threshold
            ]
            if not stacked:
                stacked = [primary]
            best = max(stacked, key=lambda t: (t.y, -abs(t.x - marker.x)))
            return best, "below-marker"
        # else: closest VALUE_RE candidate is too far below — fall through
        # to the dash check and then the same-line fallback.

    # No VALUE_RE candidate sits in the primary cell. Check for a dash
    # token directly below the marker. This handles "no revision yet"
    # drawings where the rev cell contains a literal ``-``.
    if tokens is not None:
        DASH_VALUES = {"-", "_", "\u2212", "\u2014", "\u2013"}  # ascii, unicode minus, em/en dash
        for t in tokens:
            if t.text not in DASH_VALUES:
                continue
            if t is marker:
                continue
            if t.y <= marker.y + max(marker.h, t.h) * 0.3:
                continue  # at or above marker
            if abs(t.x - marker.x) > col_tol:
                continue  # outside the marker's column
            if t.y - marker.y > primary_cell_max_dy:
                continue  # too far below — not in primary cell
            # Found one. Take the closest below marker.
            return t, "below-marker-dash"
...
def _extract_from_marker(
    ...
    *,
    ...
    if region == "br":
        ...
        chosen = _pick_latest_in_title_block(raw, marker, tokens=tokens)
        region_tag = "br_title"
...
{
  "rev_value":   "<value from the set above, or empty string, or '-' for no-revision-yet drawings>",
...
# Dash / no-revision-yet drawings

Some new drawings have NOT been revised yet — the Rev cell contains a literal `-` (a single hyphen) or `_` (underscore), often with a date column also showing `-`. In that case, return `rev_value = "-"` with high confidence. This is a legitimate value meaning "no revision yet", not a "couldn't read" situation. Do NOT return `""` or `"1"` from "Sheet 1 of N" when the Rev cell clearly shows a dash."""
...
def _validate_gpt_payload(data: Dict[str, Any]) -> Tuple[bool, str]:
    ...
    # Dash/underscore values indicate "no revision yet" (new drawings on
    # the Losser-template site). These are legitimate values, not invalid.
    _DASH_REV_VALUES = {"-", "_", "\u2212", "\u2014", "\u2013"}
    # Empty string is a legal value (meaning "blank/unknown").
    if value != "" and value not in _DASH_REV_VALUES and not VALUE_RE.match(value):
