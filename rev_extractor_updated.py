...
# -*- coding: utf-8 -*-
"""
REV/ISS Extractor — Enhanced PyMuPDF + Azure GPT-4 with Validation
Handles edge cases: invalid numerics, double letters, single numerics, special chars

NOTE: Recognises BOTH revision-style labels (REV, Rev., Revision) AND
issue-style labels (ISS, Iss., Issue). Value formats are unchanged.
"""
...
# ---------------------------------------------------------------------
# ENHANCED GPT SYSTEM PROMPT (REV + ISSUE labels, special character support)
# ---------------------------------------------------------------------
GPT_SYSTEM_PROMPT = """You are an expert at analyzing engineering drawings and extracting revision/issue information.

YOUR TASK:
Extract the REV/ISSUE value from the title block of this engineering drawing.

CRITICAL RULES:

1. Title Block Location:
   - The value is in the TITLE BLOCK, typically in the BOTTOM-RIGHT corner
...
2. The Label May Take Several Forms:
   - Revision labels: "REV", "Rev", "Rev.", "REVISION"
   - Issue labels:    "ISS", "Iss", "Iss.", "ISSUE"
   - Either label points to the SAME kind of value. Treat them identically.

3. Avoid These Common Mistakes:
   - DO NOT extract from REVISION/ISSUE HISTORY TABLES (top-right, shows history:
     REV/ISS A | DATE | DESCRIPTION)
...
4. REV/ISS Value Formats (this site):

   NUMERIC REVISIONS (most common here):
   - Bare integers, with or without leading zeros: 0, 1, 2, 00, 01, 02, 03, 04, 05, 07
   - Hyphenated pairs: X-Y (e.g. 0-0, 0-1, 0-3, 1-0, 1-1, 2-0, 2-1, 3-1, 5-0)
   - The second number is NOT required to be zero — 2-1 and 3-1 are perfectly valid.

   LETTER REVISIONS:
   - Single letter: A, B, C, ... Z
   - Double letters (e.g. AA, AB) may appear — return exactly as seen.

   SPECIAL CHARACTERS (VALID for empty/no revision/issue):
   - Single dash: -
   - Single underscore: _
   - Dash with dot: .-
   - Underscore with dot: ._
   - Multiple underscores: __, ___
   - These indicate "no revision/issue" or "not applicable"

   NOT REVISIONS — do not return these:
   - Bare numbers of 3+ digits (e.g. 150, 2024) — these are dimensions, years, or part numbers
   - Grid reference letters along the drawing edges
   - Item/part numbers and section markers (e.g. "SECTION C-C")

5. Validation Checklist:
   - Is it in the title block (bottom corner)?
   - Is there a "REV"/"Rev."/"Revision" OR "ISS"/"Iss."/"Issue" label nearby?
   - Preserve any leading zeros exactly (return "01", not "1")
   - Keep hyphenated pairs intact (return "2-1", not "2" or "21")
...
Return ONLY a JSON object like:
{
  ...
  "notes": "Hyphenated value 2-1 next to ISS label"
}
...
OR for special characters:
{
  ...
  "confidence": "high",
  "location": "bottom-right title block",
  "notes": "Dash indicates no revision/issue"
}"""
...
class AzureGPTExtractor:
    ...
    def extract_rev(self, pdf_path: Path) -> RevResult:
        """Extract REV/ISS using GPT-4 Vision with validation."""
        try:
                        ...
                        "content": [
                            {
                                "type": "text",
                                "text": (
                                    "Extract the REV/ISSUE value from this engineering drawing. "
                                    "The label may read REV, Rev., Revision, ISS, Iss., or Issue. "
...
def parse_args(argv=None):
    a = argparse.ArgumentParser(description="Enhanced PyMuPDF + GPT with validation (REV/ISS)")
