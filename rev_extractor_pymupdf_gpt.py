...
# -*- coding: utf-8 -*-
"""
REV Extractor — PyMuPDF + OpenAI GPT-4o Fallback
Hybrid: Try native extraction first, fall back to GPT-4o Vision for failures
"""
...
LOG = logging.getLogger("rev_extractor_gpt_hybrid")
...
class AzureGPTExtractor:
    def __init__(self, endpoint: str, api_key: str, deployment_name: str = "gpt-4o"):
        ...
        self.client = AzureOpenAI(
            ...
            api_key=api_key,
            api_version="2024-02-15-preview"  # Vision-capable version
        )
...
def run_hybrid_pipeline(
    ...
    2. Fall back to GPT-4o Vision (robust, paid)
    """
    ...
    for pdf_path in tqdm(pdfs, desc="Processing PDFs"):
        try:
            ...
            # Step 2: Fall back to GPT-4o
            LOG.info(f"→ Using GPT-4o for {pdf_path.name}")
            gpt_used += 1
    ...
    try:
        ...
        LOG.info(f"Stats: Native={native_success}, GPT-4o={gpt_used}, Cost≈${gpt_used * 0.01:.2f}")
    except Exception as e:
...
def parse_args(argv=None):
    a = argparse.ArgumentParser(description="PyMuPDF + GPT-4o hybrid extraction")
