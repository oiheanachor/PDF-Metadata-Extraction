...
def is_plausible_rev_value(v: str) -> bool:
    ...
    Returns True if valid, False if suspicious/needs verification.
    
    CRITICAL: Special characters return False because they need
    GPT verification (might be placeholders, not actual values).
    """
...
def is_suspicious_rev_value(v: str) -> bool:
    ...
    Special characters ALWAYS need verification because they might be
    placeholder lines in the REV field, not the actual value.
    """
    s = norm_val(v)
    ...
    # CRITICAL: ALL special characters need GPT verification
    # They might be placeholder lines (REV: ___) not actual values
