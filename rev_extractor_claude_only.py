...
# -*- coding: utf-8 -*-
"""
REV Extractor — Only Azure Claude Vision
All PDFs processed with Claude Vision API
Simple, reliable, but higher cost
"""
...
LOG = logging.getLogger("rev_extractor_claude_only")
...
def run_claude_only_pipeline(
    ...
    """
    Claude-only pipeline: All PDFs processed with Claude Vision.
    Advantages:
    - 100% reliable
    - Handles all edge cases (scanned, A1, complex layouts)
    - No preprocessing needed
    
    Cost: ~$0.015-0.02 per page
    """
    ...
    for pdf_path in tqdm(pdfs, desc="Processing with Claude"):
        try:
    ...
    try:
        ...
        LOG.info(f"Total cost: ≈${len(pdfs) * 0.015:.2f} (at $0.015/page)")
    except Exception as e:
...
def parse_args(argv=None):
    a = argparse.ArgumentParser(description="Claude-only REV extraction")
...
def main(argv=None):
    ...
    return run_claude_only_pipeline(
        args.input_folder,
        args.output,
        args.azure_endpoint,
        args.azure_key,
